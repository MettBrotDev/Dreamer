...
class BinaryAutoencoder(nn.Module):
    ...
    def normalize(self, x):
        mean = torch.tensor(config.MEAN, dtype=torch.float32, device=config.DEVICE)
        std = torch.tensor(config.STD, dtype=torch.float32, device=config.DEVICE)
        return (x - mean) / std
...
def compute_kl_divergence(q_probs, p_probs, steps, clip_threshold=config.FREE_NATS):
    ...
    if kl_clipped.isinf().any():
        ...
        # this was a problem in the beginning, but i think i fixed it now
