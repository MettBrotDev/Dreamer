...
class WorldModel:
    def __init__(self):
        ...
        self.writer = SummaryWriter(log_dir=config.LOG_DIR1)
    ...
    def initialize_weights(self, layer):
        if isinstance(layer, nn.Linear):
            ...
            if hasattr(layer, 'is_logvar') and layer.is_logvar:
                nn.init.constant_(layer.bias, 0.3)
            else:
    ...
    def computeDynamicsLoss(self, z_enc_probs, z_dyn_probs, summary=False):
        ...
        MSE_loss = F.mse_loss(z_enc_probs, z_dyn_probs)
        ...
        if summary:
            ...
            self.writer.add_scalar('Loss/MSE_Dynamics', MSE_loss, self.steps)
    ...
    def trainingRound(self, env, player2):
        ...
        for i in range(config.TRAIN_STEPS):
            if c.item() != 0: 
                env.reset()
            a1 = self.agent.act(self.h, self.z)
    ...
    def loadModel(self):
        if not os.path.isfile(config.FILE_NAME2):
    ...
    # Not updated to the new architecture
    def test_run(self):
        ...
        _, z_probs, _, _ = self.autoencoder.forward(obs_tensor)
        self.z = (z_probs > 0.5).float()
        self.h = torch.zeros(config.SNET_NUM_LAYERS, 1, config.H_SIZE)
        c = torch.tensor([0], dtype=torch.float32)

        self.dream(c, training=False)
        ...
        for step in range(100):
            if step in random_steps:   
                ...
                obs_tensor = torch.tensor(obs, dtype=torch.float32)
                _, z_probs, obs_rec, _ = self.autoencoder.forward(obs_tensor)
                self.z = (z_probs > 0.5).float()
                diff = torch.sum(torch.abs(obs_tensor - obs_rec))
                print("Reconstruction difference:", diff)
                ...
                for seq in self.dream_space:
                    h_pred, z_sample, z_dyn_probs, r_mean, r_logvar, c_sample, c_probs = seq[0]
                    z = (z_dyn_probs > 0.5).float()
