...
class WorldModel:
    ...
    def dream(self, c):
        ...
        for i in range(config.DREAM_STEPS):
            ...
            dreamed_sequence.append((h_next, z_embedded, z_probs, r_mean, r_logvar, c_sample, c_probs, a))

            # Since we are no longer training on multi step dreams we detach all further dreams from eachother
            h_prev = h_next.clone().detach()
            z_prev = z_embedded.clone().detach()
            if i == 0:
                self.h_next = h_next
                self.c_next = c
    ...
    def trainingRound(self, env):
        ...
        for i in range(config.TRAIN_STEPS):
            ...
            action = self.action_next.cpu().detach().numpy()
